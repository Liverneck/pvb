...
SWEP.PrintName				= "Laevatein"		-- Weapon name (Shown on HUD)
...
SWEP.Type	= "Transcendent Grade Pistol"
SWEP.Primary.Knockback = 0 

--[[WEAPON HANDLING]]--

--Firing related
SWEP.Primary.Sound 			= Sound("Laevatein.Fire")				-- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("Laevatein.Fire") -- This is the sound of the weapon, when silenced.
...
SWEP.ViewModel			= "models/weapons/tfa_cso/c_laevatein.mdl" --Viewmodel path
...
SWEP.WorldModel			= "models/weapons/tfa_cso/w_laevatein.mdl" -- Worldmodel path
...
SWEP.MuzzleFlashEffect = "muz_laevatein_a" --Change to a string of your muzzle flash effect.  Copy/paste one of the existing from the base.
SWEP.MuzzleFlashEffectSilenced = "cso_muz_laevatein_b"

--Tracer Stuff

SWEP.Tracer				= 0		--Bullet tracer.  TracerName overrides this.
SWEP.TracerName 		= "cso_tra_laevatein_a"	--Change to a string of your tracer name.  Can be custom.
...
if CLIENT then
	SWEP.WepSelectIconCSO = Material("vgui/killicons/tfa_cso_laevatein")
...
function SWEP:Think2(...)
	if self:GetStatus() == TFA.Enum.STATUS_SILENCER_TOGGLE and self:GetStatusEnd() <= CurTime() then
		...
		if self:GetSilenced() then
			...
			self.MuzzleFlashEffect = "muz_laevatein_a"
			self.Primary_TFA.ImpactEffect = nil
			self.TracerName = "cso_tra_laevatein_a"
			self:ClearStatCache()
		else
			self.TracerName = nil
			...
			self.Primary_TFA.Spread = 0.010
			self.Primary_TFA.AmmoConsumption = 2
			self.Primary_TFA.NumShots = 1
			self.MuzzleFlashEffectSilenced = "cso_muz_laevatein_b"
			self.Primary_TFA.ImpactEffect = "exp_laevatein"
			self.TracerName = "tra_laevatein_b"
			self:ClearStatCache()
		end
	end
